describe('ApiTesting_Demo', ()=>{
  ...
  it('Test 2 - Request Get Succes', ()=>{

    cy.request({method: 'GET', 
                url:'https://petstore.swagger.io/v2/store/order/77', 
                failOnStatusCode: false, 
                headers: {'Content-Type': 'application/json'}}).then((response)=>{
                    expect(response).to.have.property('headers')
                    expect(response.status).to.eq(200)
                    console.log('StatusCode: ',response.status)
                    expect(response.body).to.have.property('id', 77)
                    expect(response.body).to.have.property('petId', 77)
  ...
  it('Test 4 - Request Delete Success', ()=>{
    ...
    cy.request({method: 'DELETE', 
                ...
                headers: {'Content-Type': 'application/json'}}).then((response)=>{
                    ...
                    expect(response.body).to.have.property('message', '77')
                    console.log('Response_Body: ',response.body)
    })
  })
